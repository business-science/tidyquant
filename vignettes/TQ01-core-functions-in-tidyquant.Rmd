...
- `from / to` These get passed to `start.date` and `end.date` and can be provided in "YYYY-MM-DD" format. Note that `start.date` and `end.date` from `Rblpapi` can be used. 
