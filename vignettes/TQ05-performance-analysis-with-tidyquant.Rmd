...
We'll use the `PerformanceAnalytics` function, `table.CAPM`, to evaluate the returns of several technology stocks against the SPDR Technology ETF (XLF). 
...
Next, we get the baseline prices. We'll use the XLF. Note that there is no need to group because we are just getting one data set.
...
### Step 2A: Transform to returns

Using the `tidyverse` split, apply, combine framework, we can transform groups of stocks by first "grouping" with `group_by` and then applying a transformation function using `tq_transform`. We use the `quantmod` function `periodReturn` as the transformation function. We pass along the arguments `period = "monthly"` to return the results in monthly periodicity. Last, we use the `col_rename` argument to rename the output column. 
...
Step 3A can be skipped because we are only interested in the Sharpe Ratio for individual stocks.
...
Let's recreate the CAPM analysis in the "Quick Example" this time comparing a portfolio of technology stocks to the SPDR Technology ETF (XLR).

#### Steps 1A and 2A: Asset Period Returns

This is the same as what we did previously to get the monthly returns for groups of individual stock prices. We use the split, apply, combine framework using the workflow of `tq_get`, `group_by`, and `tq_transform`.
...
stock_returns_monthly %>%
    tq_portfolio(assets_col = symbol, returns_col = Ra, 
                 weights = wts_map, col_rename = "Ra_using_wts_map")
```
...
This is where it gets fun. If you picked up on Step3A, Method 2 (mapping weights), this is just an extension for multiple portfolios.
...
portfolio_returns_monthly_multi <- stock_returns_monthly_multi %>%
    tq_portfolio(assets_col = symbol, returns_col = Ra, 
                 weights = weights_table, col_rename = "Ra")
...
We've only scratched the surface of the analysis functions available through `PerformanceAnalytics`. The list below includes all of the compatible functions.

```{r}
tq_performance_fun_options()
```

The table functions are the most useful to get a cross section of metrics. We'll touch on a few. We'll also go over `VaR` and `SharpeRatio` as these are very commonly used as performance measures. 
...
# Customizing tq_portfolio and tq_performance using the ...
...
## Customizing the Portfolio Aggregation
...
This is good, but we want to see how our $10,000 initial investment is growing. This is simple with a few changes to the underlying `Return.portfolio` arguments. All we need to do is add these as additional parameters to `tq_portfolio`!

```{r}
wts <- c(0.5, 0, 0.5)
portfolio_growth_monthly <- stock_returns_monthly %>%
    tq_portfolio(assets_col = symbol, returns_col = Ra, 
                 weights = wts, col_rename = "investment.growth",
                 wealth.index = TRUE) %>%
...
portfolio_growth_monthly_multi <- stock_returns_monthly_multi %>%
    tq_portfolio(assets_col = symbol, returns_col = Ra, 
                 weights = weights_table, col_rename = "investment.growth",
                 wealth.index = TRUE) %>%
    mutate(investment.growth = investment.growth * 10000)
```
...
## Customizing the Sharpe Ratio

Finally, the same concept of passing arguments works with all the `tidyquant` functions that are wrappers including `tq_transform`, `tq_mutate`, `tq_performance`, etc. Let's use a final example with the `SharpeRatio`, which has the following arguments.
...
```{r}
RaRb_multiple_portfolio %>%
    tq_performance(Ra = Ra, performance_fun = SharpeRatio)
```
...
As expected, the Sharpe Ratio goes down (indicating higher risk) as the confidence interval increases. 
