...
Using the `tidyverse` split, apply, combine framework, we can transform groups of stocks by first "grouping" with `group_by` and then applying a transformation function using `tq_transmute`. We use the `quantmod` function `periodReturn` as the transformation function. We pass along the arguments `period = "monthly"` to return the results in monthly periodicity. Last, we use the `col_rename` argument to rename the output column. 

```{r}
stock_returns_monthly <- stock_prices %>%
    group_by(symbol) %>%
    tq_transmute(ohlc_fun = Ad, transform_fun = periodReturn, 
