...
This vignette aims to cover two aspects of performance analysis: 
...
Now, we combine the two data sets using the "data" field using `left_join` from the `dplyr` package. Review the results and see that we still have three groups of returns, and columns "Ra" and "Rb" are side-by-side.
...
##### Aggregating a Portfolio using Method 1, Vector of Weights
...
##### Aggregating a Portfolio using Method 1, Two Column Tibble with Symbols and Weights
...
Second, get baseline asset ruturns, which is the exact same as Steps 1B and 2B from the Single Portfolio example.
...
1. The length (number of rows) has trippled. This is the essence of `tq_repeat_df`: it grows the data frame length-wise, repeating the data frame `n` times. In our case, `n = 3`.
...
portfolio_returns_monthly_multi <- stock_returns_monthly_multi %>%
    tq_portfolio(assets_col = symbol, returns_col = Ra, weights = weights_table,
                 col_rename = "Ra")
