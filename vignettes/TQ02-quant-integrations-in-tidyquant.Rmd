...
`tq_transmute_fun_options()` returns a list the __compatible transform functions__ by each package. We'll discuss these options by package briefly.
...
The `xts::to.period` function is used for periodicity aggregation (converting from a lower level periodicity to a higher level such as minutes to hours or months to years). Because we are seeking a return structure that is on a different time scale than the input (daily versus Weekly), we need to use a transform function. We select `tq_transmute` and pass the open, high, low, close and volume using OHLC format via `ohlc_fun = OHLCV`. Looking at the documentation for `to.period`, we see that it accepts a `period` argument that we can set to `"weeks"`. The result is the OHLCV data returned with the dates changed to one day per week.

```{r}
AAPL %>%
    tq_transmute(ohlc_fun = OHLCV, transform_fun = to.period, period = "weeks")
```
...
```{r}
AAPL %>%
    tq_transmute_xy(x = adjusted, transform_fun = to.period, period = "weeks")
```
...
The `xts::apply.quarterly()` function that is part of the period apply group can be used to apply functions by quarterly time segments. Because we are seeking a return structure that is on a different time scale than the input (quarterly versus daily), we need to use a transform function. We select `tq_transmute` and pass the close price using OHLC format via `ohlc_fun = Cl`, and we send this subset of the data to the `apply.quarterly` function via the `transform_fun` argument. Looking at the documentation for `apply.quarterly`, we see that we can pass a function to the argument, `FUN`. We want the maximum values, so we set `FUN = max`. The result is the quarters returned as a date and the maximum closing price during the quarter returned as a double. 

```{r}
AAPL %>%
    tq_transmute(ohlc_fun = Cl, transform_fun = apply.quarterly, FUN = max)
```
