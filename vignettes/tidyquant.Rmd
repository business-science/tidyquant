...
Once we have a section, we can quickly access any of the sub.sections and/or categories. For example, we can easily chart the ratios in the "Liquidity/Financial Health" `sub.section` of the "Financial Health" `section`:

```{r}
aapl_key_ratios %>%
    filter(section == "Financial Health") %>%
    unnest() %>%
    filter(sub.section == "Liquidty/Financial Health") %>%
    ggplot(aes(x = date, y = value, col = category)) + 
    geom_line() +
    labs(title = "Financial Health Ratios for AAPL", x = "", y = "") +
    lims(y = c(0, 4))
```
